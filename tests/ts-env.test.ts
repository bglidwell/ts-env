...
describe("Env helper", () => {
  describe("env without cast", () => {
    test("Returns undefined if env var is not defined", () => {
      expect(envHelper.env("NO_VAR")).toBeUndefined();
    });

    test("Returns env var as is", () => {
      process.env.WITH_VAR = "";
      expect(envHelper.env("WITH_VAR")).toEqual("");

      process.env.WITH_VAR = "test";
      expect(envHelper.env("WITH_VAR")).toEqual("test");
    });
  });

  describe("env with integer cast", () => {
    test("Returns the default if env not defined", () => {
      expect(envHelper.int("INT_VAR_NOT_EXIST", 1)).toEqual(1);
    });

    test("Returns a valid int", () => {
      process.env.INT_VAR = "123";
      expect(envHelper.int("INT_VAR", 1)).toEqual(123);
    });
  });
  ...
  describe("env with float cast", () => {
    test("Returns a valid float when possible", () => {
      ...
      expect(envHelper.float("FLOAT_VAR", 123.45)).toEqual(123.45);
    });

    test("Returns the default if env not defined", () => {
      expect(envHelper.float("FLOAT_VAR_NOT_EXIST", 1.0)).toEqual(1.0);
    });
  });
  ...
  describe("env with boolean cast", () => {
    test.each(["", "1", "-1", "false"])(
      ...
      (value) => {
        ...
        expect(envHelper.bool("NOT_TRUE", true)).toEqual(false);
      }
    );

    test('Returns true when using "true"', () => {
      process.env.TRUE_VAR = "true";
      expect(envHelper.bool("TRUE_VAR", false)).toEqual(true);
    });
  ...
  describe("env with json cast", () => {
    test("Throws if var is not a valid json", () => {
      ...
      expect(envHelper.json("JSON_VAR", {})).toEqual({});
    });
  ...
  describe("env with date cast", () => {
    test("Returns InvalidDate if var is not castable", () => {
      ...
      expect(envHelper.date("NOT_DATE_VAR", new Date())?.getTime()).toEqual(
        Number.NaN
      );
    });
    ...
    test("Returns a valid date when possible", () => {
      ...
      expect(envHelper.date("DATE_VAR", new Date())).toEqual(
        new Date(2010, 1, 21, 12, 34, 12)
      );
    });

    test("Parses ISO ", () => {
      process.env.DATE_VAR_ISO = "2010-02-21T12:34:12.016Z";
      expect(envHelper.date("DATE_VAR_ISO", new Date())).toEqual(
        new Date("2010-02-21T12:34:12.016Z")
      );
    });
  });
});
